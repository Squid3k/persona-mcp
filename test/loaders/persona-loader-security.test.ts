...
describe('PersonaLoader Security', () => {
  ...
  describe('Path Traversal Protection', () => {
    it('should reject loading files with path traversal attempts', async () => {
      ...
      const result = await loader.loadPersonaFromFile(maliciousPath, 'user', testDir);
      
      expect(result.isValid).toBe(false);
      // Check that at least one error mentions path traversal or invalid extension
      const hasRelevantError = result.validationErrors?.some(err => 
        err.includes('Path traversal') || err.includes('Invalid file extension')
      ) ?? false;
    ...
    it('should reject files outside the base directory', async () => {
      ...
      try {
        const result = await loader.loadPersonaFromFile(outsideFile, 'user', testDir);
        expect(result.isValid).toBe(false);
      } finally {
...
`;
      ...
      const result = await loader.loadPersonaFromFile(validFile, 'user', testDir);
    ...
    it('should reject files with invalid extensions', async () => {
      ...
      const result = await loader.loadPersonaFromFile(invalidFile, 'user', testDir);
      expect(result.isValid).toBe(false);
      const hasExtensionError = result.validationErrors?.some(err => 
...
`;
      ...
      const result = await loader.loadPersonaFromFile(nestedFile, 'user', testDir);
      expect(result.isValid).toBe(true);
